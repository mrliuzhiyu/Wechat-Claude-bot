...
- **실시간 진행 상황** — Claude Code가 파일 읽기, 코드 작성, 명령 실행 시 실시간으로 WeChat에 진행 상황 전송
- **완전한 코드 능력** — Claude Code는 완전한 권한 보유: 파일 읽기/쓰기, 터미널 명령 실행, 코드 검색, 패키지 설치
- **다중 사용자 세션 격리** — 각 WeChat 사용자는 독립 세션, 대화 컨텍스트 연속, 상호 간섭 없음
- **음성 메시지 지원** — WeChat 음성 메시지 지원 (음성-텍스트 변환 활성화 필요)
- **스마트 메시지 분할** — 긴 응답은 코드 블록 경계에서 자동으로 스마트 분할, 코드 중간에서 끊기지 않음
- **Markdown 변환** — Claude의 Markdown 출력을 WeChat 친화적인 텍스트로 자동 변환
- **자동 재연결** — 세션 만료 시 자동 재로그인, 토큰 영구 저장으로 재스캔 불필요
- **동시성 제어** — 최대 3개 작업 동시 처리, 초과분은 자동 대기열
- **정상 종료** — Ctrl+C 안전 종료 지원, 자식 프로세스 자동 정리
...
현재 텍스트 메시지와 음성 메시지(음성-텍스트 변환 활성화 시)만 지원합니다. 이미지, 동영상, 파일은 아직 미지원입니다.
