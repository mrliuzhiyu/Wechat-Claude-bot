...
WeChat 공식 [OpenClaw](https://github.com/nicepkg/openclaw)는 AI 모델을 로컬에서 실행하고 ClawBot 프로토콜로 WeChat에 연결하는 프레임워크입니다. 강력하지만 분명한 문제점이 있습니다:

### OpenClaw의 문제점

- **토큰 소비가 막대** — OpenClaw는 대화마다 Claude API를 직접 호출하여 토큰 단위 과금. 복잡한 대화는 쉽게 수만 토큰을 소비하며 비용 통제 불가
- **설치가 복잡** — OpenClaw 프레임워크 설치, API Key 설정, 모델 파라미터 조정, 의존성 충돌 해결 필요. 비전문가에게는 진입 장벽이 높음
- **유지보수 비용이 높음** — 프레임워크 업데이트 빈번, 복잡한 의존성 체인, 버전 호환성 문제. 로컬 모델 실행에는 GPU/메모리 요구
- **채팅만 가능** — 완전히 설정해도 OpenClaw는 본질적으로 챗봇. 텍스트 대화만 가능, 컴퓨터나 프로젝트 파일 조작 불가

### 본 프로젝트의 장점

본 프로젝트는 **OpenClaw 프레임워크를 사용하지 않습니다**. WeChat iLink Bot 프로토콜 + 로컬 Claude Code CLI에 직접 연결하여 위 문제를 근본적으로 해결:

| 비교 항목 | OpenClaw 직접 연결 | 본 프로젝트 (WeChat Claude Code Bot) |
|----------|-------------------|-------------------------------------|
| **토큰 비용** | 대화마다 API 토큰 소비, 종량 과금, 비용 불투명 | **추가 토큰 제로**. Claude Code CLI 구독 한도 사용, API 호출 없음 |
| **설치 난이도** | 프레임워크 + API Key + 의존성 해결 | **3단계**: `git clone` → `npm install` → `npm start`, API Key 불필요 |
| **유지보수** | 잦은 업데이트, 의존성 충돌, 버전 문제 | **유지보수 제로**. 경량 의존성 2개만 (dotenv + qrcode-terminal) |
| **기능 범위** | 텍스트 채팅만 | **컴퓨터 완전 제어**: 파일 읽기/쓰기, 명령 실행, 코드 검색, Git 작업 |
| **프로젝트 접근** | 로컬 파일 시스템 접근 불가 | 실제 프로젝트 코드 직접 조작, 변경 즉시 반영 |
| **명령 실행** | 미지원 | 모든 터미널 명령 (npm, git, docker, python 등) |
| **컨텍스트** | 현재 채팅 텍스트만 | 전체 프로젝트 디렉토리가 컨텍스트 |
| **도구 호출** | 내장 도구 없음 | 10개 이상 내장 도구: Read, Write, Edit, Bash, Glob, Grep 등 |
| **진행 피드백** | 없음 | 모든 작업의 실시간 진행 알림 |
| **미디어 지원** | 제한적 | 이미지/파일/동영상 수신, Claude Code 분석; `/send`로 파일 전송 |
| **모델 전환** | 설정 변경 + 재시작 필요 | `/model` 명령으로 Sonnet/Opus/Haiku 즉시 전환 |
| **코드 규모** | 거대한 프레임워크, 수천 파일 | **핵심 파일 3개** (index.js + weixin-api.js + claude-code.js) |

### 한마디로

> **OpenClaw** = 무거운 프레임워크 + API 종량 과금 + 채팅만
>
> **본 프로젝트** = 파일 3개 + 추가 비용 제로 + AI 프로그래머 원격 제어

### 어떤 것을 선택할까

- **WeChat에서 AI와 채팅만** 하고 싶다면 OpenClaw도 가능 (토큰 비용 부담)
- **WeChat으로 PC 원격 제어, 코드 수정, 명령 실행, 프로젝트 관리**를 원한다면 본 프로젝트가 최선
- **Claude Code 구독이 있다면** 추가 비용 제로로 WeChat 원격 제어 실현
