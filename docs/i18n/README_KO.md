...
## 왜 OpenClaw를 직접 사용하지 않는가?

WeChat 공식 [OpenClaw](https://github.com/nicepkg/openclaw)는 다양한 모델 백엔드를 지원하고 ClawBot 프로토콜로 WeChat에 연결하는 다기능 AI 에이전트 프레임워크입니다. 커스텀 AI 기능이 필요한 시나리오에 적합한 플랫폼 수준의 솔루션입니다.

하지만 목표가 **WeChat으로 로컬 Claude Code를 원격 제어하는 것**이라면 OpenClaw는 최적의 선택이 아닙니다. 핵심 차이점은 다음과 같습니다:

### 경량 vs 중량급

| 비교 항목 | OpenClaw | 본 프로젝트 (WeChat Claude Code Bot) |
|----------|----------|-------------------------------------|
| **포지셔닝** | 범용 AI 에이전트 프레임워크, 다양한 모델과 플러그인 지원 | 하나에 집중: WeChat과 Claude Code CLI 연결 |
| **설치** | 프레임워크 + API Key + 모델 설정 + 플러그인 시스템 + 의존성 체인 | **3단계**: `git clone` → `npm install` → `npm start` |
| **의존성** | 프레임워크 방대, 의존성 다수, 설치 시 문제 발생 잦음 | **의존성 2개만** (dotenv + qrcode-terminal) |
| **유지보수** | 프레임워크 업데이트 빈번, 버전 호환 문제 발생 | 유지보수 거의 불필요, 코드 간결하고 투명 |
| **코드 규모** | 완전한 프레임워크, 수천 파일 | **핵심 파일 4개**, 총 약 1000줄 |
...
| | OpenClaw | 본 프로젝트 |
|-|----------|-----------|
| **과금 방식** | 대화마다 Claude API 호출, 토큰 단위 과금 | 로컬 Claude Code CLI 사용, 구독 한도로 운영 |
| **비용 특성** | 긴 대화, 코드 분석, 다단계 상호작용에서 토큰 소비 크고 비용 예측 불가 | **추가 토큰 비용 제로**, 기존 Claude Code 구독이 있으면 그것으로 충분 |
| **API Key** | 필수 | 불필요 |

### Claude Code의 고유 역량

본 프로젝트가 API를 직접 호출하지 않고 Claude Code CLI에 연결하는 이유는 Claude Code가 **API 호출만으로는 대체할 수 없는 역량**을 제공하기 때문입니다:

- **완전한 컴퓨터 제어** — 임의의 파일 읽기/쓰기, 임의의 터미널 명령 실행, 코드베이스 전체 검색. 단순한 텍스트 대화가 아님
- **프로젝트 수준 컨텍스트** — 전체 프로젝트 디렉토리가 Claude Code의 컨텍스트, 코드베이스 구조 전체를 이해
- **10개 이상 내장 도구** — Read, Write, Edit, Bash, Glob, Grep, WebSearch 등 완전한 도구 체인
- **실제 코드 작업** — 실제 파일 직접 수정, Git 명령 실행, 의존성 설치, 변경 즉시 반영
- **실시간 진행 상황** — 모든 작업(파일 읽기, 명령 실행, 코드 편집)을 실시간으로 WeChat에 전송
- **세션 지속** — 독립 세션 관리, 대화 컨텍스트가 턴을 넘어 지속

이러한 역량은 API 호출 + 자체 도구 체인 구축으로도 구현 가능하지만, Claude Code CLI는 이미 완성된 성숙한 제품입니다. 바퀴를 다시 발명할 필요가 없습니다.

### 한마디로

> **OpenClaw** = 범용 시나리오용 다기능 AI 프레임워크. 하지만 무겁고, 비용이 들고, 설정이 복잡
>
> **본 프로젝트** = 파일 4개, 추가 비용 제로, 하나에 집중: WeChat에서 Claude Code 원격 제어

### 선택 기준

- **범용 AI WeChat 봇 구축**, 다양한 모델과 커스텀 플러그인 필요 → OpenClaw
- **WeChat으로 PC 원격 제어, 코드 수정, 명령 실행** → 본 프로젝트
- **Claude Code 구독 보유**, 추가 비용 없이 WeChat 원격 제어 → 본 프로젝트
