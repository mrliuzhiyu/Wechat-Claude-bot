...
- **实时进度反馈** — Claude Code 读文件、写代码、执行命令时，实时推送进度到微信
- **完整代码能力** — Claude Code 拥有完整权限：读写文件、执行终端命令、搜索代码、安装依赖
- **多用户会话隔离** — 每个微信用户独立会话，对话上下文连续，互不干扰
- **语音消息支持** — 支持微信语音消息（需开启语音转文字）
- **智能消息拆分** — 长回复自动按代码块边界智能拆分，不会从代码中间截断
- **Markdown 转换** — 自动将 Claude 的 Markdown 输出转为微信友好的纯文本格式
- **自动重连** — Session 过期自动重新登录，Token 持久化免重复扫码
- **并发控制** — 最多同时处理 3 个任务，超出自动排队
- **优雅退出** — 支持 Ctrl+C 安全退出，自动清理子进程
