...
- [功能特性](#功能特性)
- [与直连 OpenClaw 的对比](#与直连-openclaw-的对比)
- [微信连接方式](#微信连接方式)
- [工作原理](#工作原理)
- [快速开始](#快速开始)
- [配置说明](#配置说明)
...
- **斜杠命令** — `/new` 重置对话、`/status` 查看状态、`/help` 获取帮助
...
微信官方的 OpenClaw (ClawBot) 允许直接在微信中对话 AI。本项目在此基础上连接 **Claude Code CLI**，带来本质性的区别：

| 对比维度 | 直连 OpenClaw | 本项目 (WeChat Claude Code Bot) |
|---------|--------------|-------------------------------|
| **能力范围** | 纯文字对话，只能聊天 | 能真正操作电脑：读写文件、执行命令、搜索代码 |
| **Token 消耗** | 每次对话消耗 API Token，按量付费 | 使用本机 Claude Code CLI，包含在你的订阅中，不额外消耗 API Token |
| **操作项目** | 无法访问本地文件 | 直接操作你的真实项目代码，改完即生效 |
| **执行命令** | 不支持 | 可执行任意终端命令（npm、git、docker 等） |
| **上下文** | 仅对话文本 | 整个项目目录都是上下文，Claude 能理解完整代码库 |
| **工具调用** | 无 | 10+ 内置工具：Read、Write、Edit、Bash、Glob、Grep、WebSearch 等 |
| **进度反馈** | 无 | 实时推送每一步操作进度到微信 |
| **Git 操作** | 不支持 | 直接 commit、push、创建分支等 |
| **安装依赖** | 不支持 | 可运行 `npm install`、`pip install` 等 |
| **多轮连续** | 有限上下文 | 独立 session 管理，对话上下文持续保持 |

### 一句话总结

> **直连 OpenClaw** = 在微信里和 AI 聊天
>
> **本项目** = 在微信里远程操控一个能读写代码、执行命令、管理项目的 AI 程序员
...
目前仅支持文字消息和语音消息（需微信开启语音转文字）。图片、视频、文件等暂不支持。
